...
$get_media = function ( string $fallback ) use ( $media_base ): string {
    ...
    return esc_url( 'https://selacloud.com/wp-content/uploads/hero/sections/sela-stats/media/' . ltrim( $fallback, '/' ) );
};
    ...
    <div class="slst-wrap">
        ...
        <div class="slst-row">
            <?php for ( $i = 1; $i <= 4; $i++ ) :
                ...
                $symbol = (string) ( $settings["stat_{$i}_symbol"] ?? '' );

                if ( $lbl === '' ) {
                    continue;
                }

                if ( $symbol === '' && ! empty( $settings["stat_{$i}_plus"] ) ) {
                    $symbol = '+';
                }

                $symbol_class = $symbol === '<' ? 'lt' : 'plus';
            ?>
                <div class="slst-item">
                    <div class="slst-num">
                        <?php if ( $symbol !== '' ) : ?>
                            <span class="slst-sym slst-sym--<?php echo esc_attr( $symbol_class ); ?>"><?php echo esc_html( $symbol ); ?></span>
                        <?php endif; ?>
